        ...
        while incercari < 50000:
            ...
            if (suma == suma_tinta if suma_tinta else r["suma_min"] <= suma <= r["suma_max"]):
                ...
                if (r["n"] == 6 and pare in [2,3,4]) or (r["n"] == 5 and pare in [2,3]):
                    ...
                    return res
        return {"eroare": "Nu s-a găsit o variantă validă la suma respectivă. Încearcă o sumă mai echilibrată."}
...
with col_set:
    ...
    if st.button("🚀 Generează Varianta", use_container_width=True):
        with st.spinner("Căutăm combinația perfectă..."):
            ...
            suma_t = suma_dorita if suma_dorita > 0 else None
            
            rezultat = MotorLoto.genereaza_varianta(joc_selectat, luna_v, an_v, suma_t)
            st.session_state['res_gen'] = rezultat
            
            if "eroare" not in rezultat:
                st.session_state['verif_gen'] = MotorLoto.analiza_performanta_istorica(
                    rezultat["numere"], rezultat["extra"], joc_selectat, luna_v, an_v
                )
...
if tot > 0:
    st.caption(f"🔍 S-au analizat **{tot} de extrageri oficiale** pentru filtrele selectate.")
    ch, cc = st.columns(2)
    with ch: 
    ...
    st.warning("⚠️ Nu există nicio extragere salvată pentru perioada pe care ai selectat-o.")
